...
12. **REOPENED again — IN PROGRESS (Claude Sonnet 5, 2026-09-10).** Prior pass (Codex) made the mobile nav a single horizontally-scrollable line specifically to solve the "five stacked bands" complaint above — but the operator then flagged that side-scroll itself as awkward, and it was quietly cutting off SOURCES/ABOUT past the right edge with no visible affordance that more nav existed. Same pass separately broke the Columns dropdown (panel anchored `absolute left-0` off a trigger button sitting at the nav's right end, pushing the panel off-screen — reported as "the columns dropdown is not working"). **Fixed this pass:** nav now wraps onto extra lines below `md` instead of scrolling (each pill has its own full border + gap so a wrapped pill never loses its left edge, unlike the old shared-border chain); `ColumnsMenu`'s dropdown now anchors `right-0` with a `max-w-[calc(100vw-2rem)]` cap so it can't run off either edge regardless of where the trigger sits. Verified in-browser (desktop width, ~1112px): nav wraps cleanly, Columns opens fully on-screen. `tsc --noEmit`/`next build` clean, pushed (`d2b4697`). **Not resolved, needs a real device repro:** operator also reported the mobile Filters bottom-sheet "appears off screen mostly." Couldn't reproduce at any width this session — the browser tool available this pass could not actually shrink the CSS viewport below ~1112px (window-resize calls report success but `window.innerWidth` never moves), so true phone-width behavior went unverified. The Filters panel's own positioning (`fixed inset-x-3 bottom-3` on mobile) looks structurally sound by code inspection and has no obvious shared root cause with the two fixes above. Needs either a real phone screenshot after this fix, or a working narrow-viewport repro tool, before touching it further — don't guess-fix without reproducing it.
