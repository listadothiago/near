...
**Active handoff — 2026-09-07, Codex (GPT-5):** simplified the PWA source mark locally to a single acid-green `N` on charcoal, rendered it for app, Apple, standard, and maskable icons, and updated the manifest to use a dedicated maskable asset. Added restrained acid-green-on-dark treatment to the header freshness stamp and card revision badge. `near-illustrator` now explicitly allows precise vector-derived abstract work as an art-direction option. **Webpack production build passed; not pushed or deployed yet.**
