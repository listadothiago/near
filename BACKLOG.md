...
**Every full rotation round must also:** check the local inbox folder (photos, documents, submissions) for anything actionable; publish at least five new curated hotels (see `docs/workflows/content-rotation.md`; ordinary standalone refreshes retain at least one attempt); run `geolocation-police` against Google Maps on every piece written or refreshed (see §1.4).
...
- **Batch continuity:** manifest currently records **20 published, 9 candidates, 1 held** (`roxie-theater-mission-san-francisco`); first remaining candidate **Sexyland World**. Older 8/30 and Bar Etienne handoffs were stale. These are manifest counts, not a new live audit. Resume P0.15 and reconcile evidence before publication; no competing batch or rotation was started here.
- **Rotation:** saved NEXT-UP and spent state preserved. Reconcile divergent tier memberships from BACKLOG §2.2 and the rotation skill with `rotation-reorder` at the next full-round start. All 27 saved Trends fetches failed DNS; one web fallback failed too. No invented demand ranking.
- **Remaining decisions:** P0.4 retains the fresh-session requirement; P1.0/n8n remains a discussion. Public persona/column launches, AR UI and live article image/headline repairs remain separately scoped below.
...
12. **DONE (Claude Sonnet 5, 2026-09-10), three rounds against real operator screenshots from an actual phone (not the session's browser tool, which could never shrink its CSS viewport below ~1112px this pass).** Round 1: nav side-scroll (from a prior Codex pass) was cutting off SOURCES/ABOUT with no affordance, and the same pass's Columns dropdown opened off-screen (`absolute left-0` off a trigger sitting at the nav's right end) — fixed by wrapping the nav instead of scrolling and anchoring the dropdown `right-0` (`d2b4697`). Round 2: the operator screenshotted the real result and called the two-line wrap itself "terrible... just make that font smaller" — reverted to one line via smaller type/padding (0.68rem->0.56rem) instead of wrapping, matching the original 2026-09-08 fix for this exact symptom. Same screenshot also caught the Filters bottom-sheet rendering below the fold requiring a scroll to reach — root cause: `fixed inset-x-3 bottom-3` anchors only the bottom edge, and Android Chrome's dynamic address bar positions bottom-anchored fixed elements against the large (toolbar-hidden) viewport, which sits below what's visible when the toolbar shows. Fixed by anchoring `fixed inset-0` (all four edges) with flex to bottom-align the panel inside (`41f432f`). Round 3, same operator screenshot batch: the tagline badge next to the wordmark was still visibly truncating ("THE ALWAYS FRESH GUIDE TO ANYWH...") — shrunk 0.62rem->0.5rem and dropped its extra letter-spacing (`4a67793`). `tsc --noEmit`/`next build` clean before all three pushes. **Still open, NOT fixed this pass:** operator separately reported "can't see the collapsed header on PWA" — scrolling down in the installed home-screen app appears to lose the sticky header entirely rather than collapsing to the one-line compact state. Investigated by code inspection only (no PWA-standalone repro available in this session's tooling): `Header.tsx`'s `compact` state only hides the nav row and tagline/freshness row, never the header itself or its `position: sticky`; no ancestor `overflow-hidden`/transform that would break sticky containment was found in `app/[locale]/layout.tsx` or `globals.css`. Root cause not identified — needs either a real installed-PWA screenshot mid-scroll (to confirm it's actually un-stickying vs. just visually blending) or a device to test on directly. Don't guess-fix this one blind.
...
15. **Finish `near-batch-30` run `2026-09-07-batch-30` (Must, active).** Resume the existing manifest: 20 marked published, 9 candidates and 1 held as of 2026-09-09; first remaining candidate Sexyland World. Reconcile actual publication and gate evidence before trusting those counts. Preserve scope, per-piece evidence and all six locales. Resolve the recorded Discover defects (P2.14/P2.15b/P2.20) before counting affected pieces as fully signed off. Do not open a competing batch.
...
26a. **Do Not Disturb #1 gallery needs better hero images (operator, 2026-09-08).** Operator review: the comic gag panel illustration (tract-panel-gag.png) "did not work" and was removed from all six locales of `content/collections/do-not-disturb-01-below-the-room` along with the file itself. Separately, of the remaining gallery photos, the operator called Central Station "cute" and the Michelberger illustration "ok," but judged the Marine Parade/Brighton streetscape (Legends Hotel) and the Hazlitt's exterior shot "bad — uninspiring" — both removed from the gallery in all six locales (prose untouched, just the two weak `<Figure>` blocks). `npx next build` verified clean after both removals. Not done, explicitly deferred by the operator ("no tokens to solve now"): finding or commissioning actually-good replacement images for Legends Hotel and Hazlitt's — either better real photography (the venues' own sites/Instagram) or a near-illustrator original — before the gallery is considered finished. near-illustrator to pick this up on a future pass.
...
32. **AR/VR article and card experience:** scope transparent AR backgrounds, readable/side-scrollable floating cards, map/ad placement and a suitable VR background; feasibility first, then device verification. Workflow checks are implemented; the AR UI is not. P1.21's manual feedback export and Discover UI defects remain open.
33. **Event badge direct navigation:** Product Trio to make a venue card's event badge link directly to its active child event; validate expiry and keyboard/touch interaction. Events police records failures but does not implement UI.
...
4. **"Do Not Disturb" accommodation column (weekly, NITE-PORTER).** Standing RADAR-X collaboration: Trendsetter and SEO pick destinations and flavour (RADAR-X co-signs publicly, SEO collaborates but does not); NITE-PORTER finds the hotels to feature (no duplicates), assigns a persona to write each missing hotel article, and writes the column about the collection. NITE-PORTER must write or collaborate on — and be credited in — every hotel article. Hotel article directives: always frontload unique selling points in copy and bullets, always keep information current. **Every full rotation/batch requires five new curated hotels; ordinary standalone refreshes attempt at least one**, per `docs/workflows/content-rotation.md`. The column exists; remaining work is ongoing publication and hotel coverage.
...
12. **Give every agent/persona its own markdown "brain" file (operator, 2026-09-07):** generalize the existing pattern (`content/parser-knowledge-base.md`, `content/near-ceo-knowledge-base.md`, `content/chief-editor-knowledge-base.md`, and the near-seo/backlink-pr/ad-sales ones requested in P3.11) across the full roster — `near-trendsetter` and `near-cmo` brains and skill hooks are DONE (Codex GPT-6, 2026-09-09), unblocking rotation. The rest of the roster remains to be scoped; no blanket rollout performed.
...
14. **PR knowledge and campaign milestones:** extend P3.8/P3.11 to evaluate 100/300/500 places per city, geolocation/app publishers and major international outlets; keep sourced targets, relationship approaches and content wishes in PR brains. Assess the operator's G1/Machado-map example, without assuming a backlink exists. Reflect any approved milestones in the dashboard artifact with tabs. No outreach authorized. Original URLs and feedback: `docs/operator-workflow-feedback-2026-09-09.md`.
15. **Tapana P. persona and column:** resolve the existing “tapana pantera” identity, rename publicly to “Tapana P.”, define a whole column and add it to rotation. Preserve stable identifiers/URLs where appropriate and supply all six locales; no rename or column launch performed in this workflow pass.
...
isn't dyke a bit offensive https://near.tips/en/place/the-bush-bushwick-new-york-city i dunno we have lots of agents to flag this sort of stuff how do our DEI and TOV feel about this, dyke bar??? 

The about page is very outdated, our focus now is freshness and proximity with a curated lens change this asap. current copy is cheesy and not actually informative # About Near

Near is the always fresh guide to anywhere — an alt-weekly for cities and everywhere they spill into, pointed at the counterculture rather than the tour bus.

It's maintained by humans working with AI, writing for people who want somewhere actually worth going, wherever they happen to be.

Every byline on this site is an AI, and every one of them says so. That's the deal: opinionated, transparently artificial, and sourced to real reporting you can click through and check.

As a push for gastronomic coverage or every hotel article we write (and we have aggressive targets for hotel content) let's also write 2 food articles, as near as possible to that hotel but maintaining curation standards. And also 1 night life article. If the same article is in both categories, that counts for both category article requirements (food and nightlife near every hotel). If a hotel is at a legitimately free of nightlife options location, fall back to another activity. If there are no cool food or activity options nearby, don't recommend that hotel for an article in the first place. 
