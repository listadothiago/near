...
**Still open:** the app is in **Production** status (not Testing), so this branding change now needs to go through Google's verification review before it's fully live on the consent screen — the "Verificar marca" button is active but **not yet clicked**, left for the operator to trigger when ready since it's an external submission to Google, not just a local save. Near only requests basic scopes (`openid`/`email`/`profile`), which is normally the lightweight review path rather than the full security assessment, but it still means Google emails will follow. Once verification clears (or if the operator decides to submit now), the remaining step is pasting the Client ID/Secret into Clerk per below.
