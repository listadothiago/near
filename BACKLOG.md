...
- [x] **GA4 added alongside Vercel (operator decision, 2026-08-31, REVERSES the prior "No GA4, no cookie banner" line above).** Operator wants the Google Analytics MCP for direct data access. GA4 is not cookieless, so this reopened the consent question the original decision existed to avoid — the operator accepted that trade-off explicitly when asked. Implementation: `components/layout/GoogleAnalytics.tsx` loads gtag only when `NEXT_PUBLIC_GA_MEASUREMENT_ID` is set AND the reader has accepted via `components/layout/ConsentBanner.tsx` (`lib/consent.ts`, localStorage-backed, GA4-only — Vercel Analytics is untouched and still needs no consent). **Still needed to activate: create the GA4 property in Google Analytics, set `NEXT_PUBLIC_GA_MEASUREMENT_ID` in Vercel env, and run the MCP setup below — none of that can be done by an agent, it requires the operator's own Google login.**
- [ ] **Google Analytics MCP setup (operator action required):** `pipx run analytics-mcp` (needs Python 3.10+); auth via `gcloud auth application-default login --scopes https://www.googleapis.com/auth/analytics.readonly,https://www.googleapis.com/auth/cloud-platform`; add to Claude's MCP config with `GOOGLE_APPLICATION_CREDENTIALS` pointing at the credentials file gcloud prints and `GOOGLE_PROJECT_ID` set to the GCP project. Needs a GA4 property to exist first (create one in analytics.google.com for near.tips, grab its Measurement ID for the env var above). Source: github.com/googleanalytics/google-analytics-mcp.
...
5. **Berry Bros. & Rudd** — 3 St James's St. 1698, still family-owned, 3 Masters of Wine, B Corp 2025; the cheap way into the expensive institution is the front door. Byline DARCY. First London travel-beat pin.
