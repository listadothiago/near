...
- **"You are here" map marker too subtle (2026-09-03, NOT started).** → `near-lead-ux` + `near-tech-lead`. **Cause located — small fix, not an investigation:** `components/map/WorldMap.tsx:48-55` (`userIcon`), rendered `:273-279`.
  - Not actually neon: a **12px dot filled with `--color-accent` (`#5c7a6c`, muted sage green)** plus a 30%-opacity halo ring. Sage over green-grey tiles is near-camouflage, and 12px is small on a dense board.
  - **Completely static** — no pulse. Every mapping product the user has muscle memory for (Google, Apple, Citymapper) animates this dot, and animation is what the eye finds. Probably the highest-value half of the fix.
  - Options cheapest first: bump size; swap off `--color-accent` to a colour *deliberately* not in the map palette so it reads as UI chrome not content; add a white/paper outer ring for contrast on any tile (standard trick, works light and dark); add a slow pulse.
  - **Constraint:** legible in **both themes**, and must not read as a *place pin* — the point is that it's the reader, not a tip. Distinguishing it from `PlaceMarker` matters as much as brightness. `prefers-reduced-motion` must disable any pulse.
