...
1. Reads every `content/places/*/meta.json` with `status: active`.
2. Fetches each one's actual `heroImage.url` and reads its true width/height
   ...
   first).
3. Checks the real, documented Google Discover image requirement: width
...
without paying for a full-catalogue crawl. Use `--limit N` for a quick
